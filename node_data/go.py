...
class GO:
    ...
    def __init__(self, organism = 9606, node_types: Union[list[GONodeType], None] = None, go_node_fields: Union[list[GONodeField], None] = None,
        ...
        Args:
            ...
            go_node_fields: node properties that will be included in graph, if defined it should be value of element(s) from GONodeField enum class
            ...
            go_edge_fields: protein-go edge properties that will be included in graph, if defined it should be value of element(s) from GOEdgeField enum class
            edge_labels: edge labels that will be included in graph, if defined it should be value of element(s) from ...EdgeLabel enum classes
    ...
    def set_node_and_edge_properties(self, go_node_fields, go_edge_fields):
        """
        Prepare node and edge properties
        """
        if go_node_fields:
            self.go_node_fields = go_node_fields
        else:
            self.go_node_fields = [field.value for field in GONodeField]
            
        if go_edge_fields:
            self.go_edge_fields = go_edge_fields
        else:
    ...
    def get_go_edges(self) -> None:
        ...
        # DOMAIN-GO EDGES       
        if any([True if et in self.edge_types else False for et in self.self.domain_to_go_edge_types]):
